...
export const useUserAccess = (userId: string): UseUserAccessResult => {
  ...
  useEffect(() => {
    ...
    const loadLinked = http
      .get<unknown>(`/api/users/${encodeURIComponent(userId)}/content`, {
  ...
  const link = useCallback(
    async (contentId: string): Promise<ContentItem> => {
      // Spec: POST /api/users/{userId}/content/{contentFileId} — no body.
      await http.post(
        `/api/users/${encodeURIComponent(userId)}/content/${encodeURIComponent(contentId)}`,
        undefined,
        { _suppressErrorToast: true },
      );
